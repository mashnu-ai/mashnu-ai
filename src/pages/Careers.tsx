...
export default function Careers() {
  ...
  return (
    <div className="relative min-h-screen bg-[#FBFBFD] text-[#1D1D1F] font-sans selection:bg-[#0071E3]/20 selection:text-[#0071E3] py-16 animate-fade-in">
      
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        
        {/* Page Header */}
        <section className="text-center max-w-3xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#0071E3]/5 border border-[#0071E3]/15 text-[10px] font-mono uppercase tracking-widest text-[#0071E3] mx-auto">
            <Sparkles className="w-3 h-3 animate-pulse" />
            <span>Join Our Systems Enclave</span>
          </div>
          <h1 className="text-4xl sm:text-5xl font-semibold tracking-[-0.02em] text-[#1D1D1F]">
            We build the systems. You own the code.
          </h1>
          <p className="text-base text-[#6E6E73] max-w-2xl mx-auto leading-relaxed">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="border border-[#E8E8ED] rounded-2xl bg-white p-6 space-y-4 hover:border-[#D2D2D7] transition-all shadow-xs">
            <div className="w-10 h-10 rounded-xl bg-[#0071E3]/5 border border-[#0071E3]/10 flex items-center justify-center">
              <Terminal className="w-5 h-5 text-[#0071E3]" />
            </div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#1D1D1F]">Uninterrupted Work</h3>
          ...
          <div className="border border-[#E8E8ED] rounded-2xl bg-white p-6 space-y-4 hover:border-[#D2D2D7] transition-all shadow-xs">
            ...
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#1D1D1F]">Total Module Ownership</h3>
            <p className="text-xs text-[#6E6E73] leading-relaxed">
          ...
          <div className="border border-[#E8E8ED] rounded-2xl bg-white p-6 space-y-4 hover:border-[#D2D2D7] transition-all shadow-xs">
            ...
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#1D1D1F]">Microsecond Precision</h3>
            <p className="text-xs text-[#6E6E73] leading-relaxed">
        ...
        <section className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E8E8ED] pb-4">
            <div>
              <h2 className="text-2xl font-semibold tracking-tight text-[#1D1D1F]">Engineering Operations</h2>
              <p className="text-xs text-[#6E6E73]">Explore active engineering and research positions within our Labs.</p>
            </div>
            
            {/* Filters */}
            <div className="flex gap-1.5 bg-[#F5F5F7] p-1 rounded-full border border-[#E8E8ED] self-start sm:self-auto">
          ...
          <div className="space-y-4">
            {filteredJobs.map(job => {
              ...
              return (
                <div 
                  key={job.id}
                  className={`border border-[#E8E8ED] rounded-2xl bg-white transition-all overflow-hidden shadow-xs ${
                    isExpanded ? 'ring-1 ring-[#0071E3]/20 border-[#0071E3]' : 'hover:border-[#D2D2D7]'
                  }`}
                >
                  {/* Job Bar Header */}
                  <div 
                  ...
                  >
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-[#6E6E73]">{job.location}</span>
                      </div>
                      <h3 className="text-base sm:text-lg font-semibold tracking-tight text-[#1D1D1F]">{job.title}</h3>
                      <p className="text-xs text-[#6E6E73] line-clamp-1">{job.description}</p>
                    </div>

                    <div className="flex items-center gap-4 shrink-0">
                      <span className="hidden sm:inline text-xs font-medium text-[#1D1D1F] bg-[#F5F5F7] px-3 py-1 rounded-full">{job.compensation.split(' • ')[0]}</span>
                      {isExpanded ? <ChevronUp className="w-5 h-5 text-[#6E6E73]" /> : <ChevronDown className="w-5 h-5 text-[#6E6E73]" />}
                    </div>
                  </div>

                  {/* Expanded Details Panel */}
                  {isExpanded && (
                    <div className="border-t border-[#E8E8ED] bg-[#FBFBFD] p-5 sm:p-6 space-y-6 animate-fade-in">
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        
                        {/* Responsibilities */}
                        <div className="space-y-3">
                          <h4 className="text-xs font-semibold uppercase tracking-wider text-[#1D1D1F]">Core Operational Directives</h4>
                          <ul className="space-y-2.5">
                            {job.responsibilities.map((resp, idx) => (
                              <li key={idx} className="flex gap-2.5 items-start text-xs text-[#6E6E73] leading-relaxed">
                        ...
                        {/* Requirements */}
                        <div className="space-y-3">
                          <h4 className="text-xs font-semibold uppercase tracking-wider text-[#1D1D1F]">Engineering Prerequisites</h4>
                          <ul className="space-y-2.5">
                      ...
                      {/* Tech stack badges */}
                      <div className="border-t border-[#E8E8ED] pt-4 flex flex-wrap gap-1.5 items-center">
                        <span className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider mr-2">Core Pipeline Stack:</span>
                        {job.stack.map(tech => (
                          <span key={tech} className="px-2.5 py-1 bg-white border border-[#E8E8ED] text-[10px] font-mono rounded-md text-[#1D1D1F] font-semibold shadow-xs">
                            {tech}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="border-t border-[#E8E8ED] pt-4 flex items-center justify-between">
                        <span className="text-xs text-[#6E6E73]">Compensation Plan: {job.compensation}</span>
                        <a 
                          href="#portal" 
                          onClick={() => setSelectedJob(job.id)}
                          className="px-4 py-1.5 rounded-full bg-[#0071E3] text-white hover:bg-[#0071E3]/90 text-xs font-medium tracking-tight transition-all shadow-xs"
                        >
        ...
        <section className="border border-[#E8E8ED] rounded-3xl bg-white p-6 sm:p-8 space-y-6 shadow-sm">
          <div className="flex items-center gap-2.5 border-b border-[#E8E8ED] pb-4">
            <div className="w-10 h-10 rounded-xl bg-[#0071E3]/5 border border-[#0071E3]/10 flex items-center justify-center">
              <Brain className="w-5 h-5 text-[#0071E3]" />
            </div>
            <div>
              <h3 className="text-base font-semibold tracking-tight text-[#1D1D1F]">Systems Alignment Diagnostic</h3>
              <span className="text-[9px] font-semibold text-[#6E6E73] uppercase tracking-wider block mt-0.5">Test your engineering values</span>
            </div>
          </div>

          {quizResult ? (
            <div className="space-y-4 py-4 text-center max-w-xl mx-auto">
              <div className="inline-flex w-12 h-12 rounded-full bg-[#0071E3]/5 border border-[#0071E3]/10 items-center justify-center text-[#0071E3] text-lg font-bold">
                ✓
              </div>
              <div className="space-y-1.5">
                <span className="text-[10px] font-semibold text-[#0071E3] uppercase tracking-wider block">Diagnostic Result</span>
                <p className="text-sm sm:text-base font-semibold text-[#1D1D1F] leading-relaxed">
                  {quizResult}
                </p>
              </div>
              <button
                onClick={resetQuiz}
                className="px-4 py-1.5 rounded-full border border-[#E8E8ED] hover:bg-[#F5F5F7] text-xs font-semibold cursor-pointer transition-colors"
              >
                Re-take Diagnostic
              </button>
            </div>
            ...
            <div className="space-y-6">
              {/* Question card */}
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider">
                  <span>Question {quizStep + 1} of {QUIZ_QUESTIONS.length}</span>
                  <span className="w-1 h-1 rounded-full bg-[#E8E8ED]" />
              ...
              <div className="grid grid-cols-1 gap-3">
                {QUIZ_QUESTIONS[quizStep].options.map((option, idx) => (
                  <button
                    ...
                    className="p-4 rounded-xl border border-[#E8E8ED] bg-[#FBFBFD] hover:bg-white hover:border-[#0071E3] text-left text-xs sm:text-sm text-[#424245] hover:text-[#1D1D1F] transition-all duration-200 cursor-pointer shadow-xs leading-relaxed"
                  >
                    {option}
                  </button>
                ))}
        ...
        <section id="portal" className="border border-[#E8E8ED] rounded-[28px] bg-[#F5F5F7] p-6 sm:p-10 relative overflow-hidden shadow-sm">
          <div className="absolute top-0 right-0 w-80 h-80 bg-[#0071E3]/5 blur-[120px] pointer-events-none" />
          
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            
            {/* Form Info Panel (Col-span 5) */}
            <div className="lg:col-span-5 space-y-4">
              <span className="text-[10px] font-mono text-[#0071E3] uppercase tracking-widest font-bold block">Live Target Portal</span>
              <h3 className="text-xl sm:text-2xl font-semibold tracking-tight text-[#1D1D1F]">Compile Your Candidacy</h3>
              <p className="text-xs text-[#6E6E73] leading-relaxed">
              ...
              <div className="p-4 rounded-xl bg-white border border-[#E8E8ED] text-xs space-y-2">
            ...
            <form onSubmit={handleLiveApply} className="lg:col-span-7 bg-white border border-[#E8E8ED] rounded-2xl p-6 sm:p-8 space-y-5 shadow-xs">
              
              <div className="space-y-1.5">
                <label className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider block">Select Target Target Role</label>
                <select
                  value={selectedJob}
                  onChange={(e) => setSelectedJob(e.target.value)}
                  className="w-full bg-[#F5F5F7] border border-[#E8E8ED] rounded-lg px-3.5 py-2 text-xs sm:text-sm text-[#1D1D1F] focus:outline-none focus:border-[#0071E3] focus:bg-white transition-colors"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider block">Candidate Name</label>
                  <input
                    type="text"
                    required
                    value={candidateName}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider block">Communication Endpoint (Email)</label>
                  <input
                    type="email"
                    required
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider block">Custom Engineering Stack & Repositories (Pitch)</label>
                <textarea
                  required
                  rows={3}
                  ...
                  className="w-full bg-[#F5F5F7] border border-[#E8E8ED] rounded-lg px-3.5 py-2 text-xs sm:text-sm text-[#1D1D1F] focus:outline-none focus:border-[#0071E3] focus:bg-white transition-colors placeholder-[#86868B]"
                />
              </div>

              {/* Simulated compilation/screening logs output console */}
              {applicationLog.length > 0 && (
                <div className="bg-[#1D1D1F] rounded-xl p-4 border border-[#424245] space-y-1 font-mono text-[10px] sm:text-[11px] text-emerald-400 overflow-x-auto shadow-inner max-h-40 overflow-y-auto">
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-full text-xs font-semibold tracking-tight transition-all flex items-center gap-1.5 cursor-pointer shadow-xs ${
                    isApplying
                      ...
                      : 'bg-[#1D1D1F] hover:bg-[#424245] text-white'
                  }`}
                >
        ...
        <section className="space-y-6 max-w-4xl mx-auto pb-12">
          <div className="text-center max-w-md mx-auto space-y-1">
            <h3 className="text-xl sm:text-2xl font-semibold tracking-tight text-[#1D1D1F]">Operational Inquiries FAQ</h3>
            <p className="text-xs text-[#6E6E73]">Transparent details regarding compensation, structure, and trial protocols.</p>
          </div>

          <div className="border border-[#E8E8ED] rounded-2xl bg-white overflow-hidden divide-y divide-[#E8E8ED] shadow-xs">
              ...
              return (
                <div key={idx} className="transition-all">
                  <button
                    ...
                    className="w-full p-5 text-left flex items-center justify-between gap-4 font-semibold text-xs sm:text-sm text-[#1D1D1F] hover:bg-[#FBFBFD] select-none cursor-pointer"
                  >
                    <span>{item.question}</span>
                    {isActive ? <ChevronUp className="w-4 h-4 text-[#0071E3]" /> : <ChevronDown className="w-4 h-4 text-[#6E6E73]" />}
                  </button>
                  {isActive && (
                    <div className="p-5 bg-[#FBFBFD] text-xs sm:text-sm text-[#6E6E73] leading-relaxed border-t border-[#E8E8ED] animate-fade-in">
                      {item.answer}
                    </div>
                  )}
                </div>
