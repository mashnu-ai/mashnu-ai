  ...
  return (
    <div className="relative min-h-screen bg-[#FBFBFD] text-[#1D1D1F] font-sans selection:bg-[#0071E3]/20 selection:text-[#0071E3] py-16">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <div className="space-y-8 max-w-3xl mx-auto animate-fade-in">
            ...
            <div className="flex items-center justify-between">
              <button 
                ...
                className="inline-flex items-center gap-2 px-4 py-2 text-xs font-sans text-[#1D1D1F] hover:text-[#0071E3] border border-[#E8E8ED] rounded-full bg-white shadow-sm transition-colors cursor-pointer"
              >
                <ArrowLeft className="w-4 h-4" />
                <span>Return to Engineering Index</span>
              </button>
              
              <div className="flex items-center gap-1 text-[10.5px] font-sans text-[#6E6E73]">
                <span>Mashnu Systems Engineering Log</span>
                <ChevronRight className="w-3.5 h-3.5" />
                <span className="text-[#0071E3] font-semibold">{selectedPost.category}</span>
              </div>
            </div>

            {/* Article Canvas */}
            <article className="border border-[#E8E8ED] rounded-[24px] bg-white p-6 sm:p-10 space-y-8 shadow-sm">
              
              {/* Header meta */}
              <div className="space-y-4 border-b border-[#E8E8ED] pb-6">
                <div className="flex items-center gap-2 text-[10px] font-sans uppercase tracking-wider text-[#6E6E73]">
                ...
                <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight text-[#1D1D1F] leading-tight">
                  {selectedPost.title}
                </h1>
                ...
                <div className="flex items-center gap-3 pt-3">
                  <img 
                    ...
                    className="w-10 h-10 rounded-full object-cover border border-[#E8E8ED] shadow-sm"
                    referrerPolicy="no-referrer"
                  />
                  <div>
                    <span className="font-sans font-semibold text-xs text-[#1D1D1F] block leading-none">{selectedPost.author.name}</span>
                    <span className="font-sans text-[11px] text-[#6E6E73] mt-0.5 block">{selectedPost.author.role}</span>
                  </div>
                </div>

                <p className="text-xs sm:text-sm text-[#1D1D1F] leading-relaxed font-sans italic border-l-2 border-[#0071E3] pl-4 py-2 bg-[#F5F5F7] rounded-r mt-4">
                  {selectedPost.summary}
                </p>
              </div>

              {/* Body Paragraphs */}
              <div className="space-y-5 text-xs sm:text-sm text-[#424245] leading-relaxed font-sans">
              ...
              {selectedPost.codeBlock && (
                <div className="space-y-1">
                  <div className="flex items-center justify-between bg-[#F5F5F7] border-t border-x border-[#E8E8ED] rounded-t-lg px-4 py-2 font-sans text-[10px] text-[#6E6E73]">
                    <span className="flex items-center gap-1.5">
                      <Terminal className="w-3.5 h-3.5 text-[#0071E3]" />
                      <span>{selectedPost.id}.{selectedPost.codeLanguage || 'ts'}</span>
                    </span>
                    <span className="text-[9px] uppercase font-semibold text-[#6E6E73]">Mashnu Compiled Spec</span>
                  </div>
                  <pre className="bg-[#1D1D1F] p-4 rounded-b-lg font-mono text-[11px] text-[#F5F5F7] overflow-x-auto leading-relaxed shadow-inner">
              ...
              <div className="space-y-3 pt-4 border-t border-[#E8E8ED]">
                <h3 className="text-xs font-sans uppercase tracking-wider text-[#6E6E73] font-semibold">Systems Diagnostic Metrics</h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 font-sans text-[11px]">
                  {selectedPost.specs.map((spec, sIdx) => (
                    <div key={sIdx} className="bg-[#F5F5F7] p-3.5 rounded-xl border border-[#E8E8ED] flex flex-col justify-center">
                      <span className="text-[#6E6E73] uppercase block tracking-wider text-[9px]">{spec.label}</span>
                      <span className="text-[#1D1D1F] font-semibold block mt-1">{spec.value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex justify-center pt-4">
              <button 
                onClick={() => setSelectedPostId(null)}
                className="inline-flex items-center gap-2 px-5 py-2.5 text-xs font-sans font-medium text-[#1D1D1F] hover:text-[#0071E3] border border-[#E8E8ED] rounded-full bg-white shadow-sm transition-colors cursor-pointer"
              >
          ...
          <div className="space-y-12 animate-fade-in">
            ...
            <div className="space-y-8 max-w-4xl mx-auto">
              {POSTS.map((post) => (
                <article 
                  key={post.id}
                  className="border border-[#E8E8ED] rounded-[24px] bg-white p-6 sm:p-8 space-y-5 hover:border-[#D2D2D7] hover:shadow-md transition-all duration-300 cursor-pointer group"
                  onClick={() => setSelectedPostId(post.id)}
                >
                  
                  {/* Card Meta with Author profile */}
                  <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#E8E8ED] pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <span className="font-sans font-semibold text-xs text-[#1D1D1F] block">{post.author.name}</span>
                        <span className="font-sans text-[10px] text-[#6E6E73] mt-0.5 block">{post.author.role}</span>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-4 text-[11px] text-[#6E6E73]">
                      <span className="px-2.5 py-0.5 text-[9px] font-semibold rounded-full bg-[#F5F5F7] text-[#1D1D1F] border border-[#E8E8ED]">
                        {post.category}
                      </span>
                  ...
                  <div className="space-y-2">
                    <h2 className="text-xl sm:text-2xl font-semibold tracking-tight text-[#1D1D1F] group-hover:text-[#0071E3] transition-colors">
                      {post.title}
                    </h2>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-[#E8E8ED]/60 text-[11px] text-[#6E6E73]">
                    <div className="hidden sm:flex gap-6">
                      {post.specs.slice(0, 2).map((s, idx) => (
                        <div key={idx} className="flex gap-1">
                          <span className="text-[#6E6E73] uppercase font-medium text-[9px]">{s.label}:</span>
                          <span className="text-[#1D1D1F] font-semibold">{s.value}</span>
                        </div>
                      ))}
                    </div>
                    
                    <span className="inline-flex items-center gap-1.5 text-xs text-[#0071E3] font-semibold tracking-tight group-hover:underline">
                      Read Breakdown
        ...
        <section className="rounded-[32px] bg-[#F5F5F7] p-8 sm:p-12 text-center space-y-6 border border-[#E8E8ED] relative overflow-hidden max-w-4xl mx-auto mt-20">
          ...
          <p className="text-sm text-[#6E6E73] max-w-xl mx-auto leading-relaxed">
          ...
          <div className="pt-2">
            <Link
              to="/contact"
              className="inline-flex px-6 py-2 rounded-full bg-[#1D1D1F] hover:bg-[#424245] text-white font-medium text-xs tracking-tight transition-colors items-center gap-1.5 shadow-sm"
            >
