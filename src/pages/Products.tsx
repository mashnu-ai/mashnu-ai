...
const PRODUCTS: ProductItem[] = [
  ...
  },
  {
    ...
    icon: <Settings className="w-5 h-5 text-[#0071E3]" />,
  ...
  },
  {
    ...
    icon: <Code className="w-5 h-5 text-[#0071E3]" />,
  ...
  return (
    <div className="relative min-h-screen bg-[#FBFBFD] text-[#1D1D1F] font-sans selection:bg-[#0071E3]/20 selection:text-[#0071E3] py-16">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
            ...
            return (
              <section 
                ...
                className={`p-8 sm:p-12 rounded-[32px] space-y-10 transition-colors duration-300 ${
                  isAlternatePhase ? 'bg-[#F5F5F7]' : 'bg-white border border-[#E8E8ED]'
                }`}
              >
                {/* Phase Title Grouping */}
                <div className="border-b border-[#E8E8ED] pb-6">
                  <span className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider block mb-1">
                    Deployment Scope
                  </span>
                  <h2 className="text-xl sm:text-2xl font-semibold tracking-tight text-[#1D1D1F]">
                    {phase}
                  </h2>
                  <p className="text-sm text-[#6E6E73] mt-1.5 max-w-3xl leading-relaxed">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
                  {phaseProducts.map((product) => (
                    <motion.div
                      ...
                      className="flex flex-col h-full bg-white rounded-[24px] overflow-hidden border border-[#E8E8ED] hover:border-[#D2D2D7] hover:shadow-md transition-all duration-300 group"
                    >
                      {/* Image/Render Area at top */}
                      <div className="h-48 sm:h-56 relative overflow-hidden bg-[#F5F5F7] border-b border-[#E8E8ED]">
                        <img 
                        ...
                        <div className="absolute top-4 left-4">
                          <span className="px-2.5 py-1 text-[9px] font-semibold rounded-full bg-[#1D1D1F] text-white tracking-wide uppercase">
                            {product.status}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="p-6 sm:p-8 flex-grow flex flex-col justify-between space-y-6">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2.5">
                            <span className="text-[#0071E3] shrink-0">{product.icon}</span>
                            <h3 className="text-lg font-semibold text-[#1D1D1F] tracking-tight">{product.title}</h3>
                          </div>
                        ...
                        <div className="space-y-4 pt-4 border-t border-[#E8E8ED]">
                          <div className="space-y-2.5 text-xs text-[#424245]">
                            <div>
                              <strong className="text-[#1D1D1F] font-medium block">How it works:</strong>
                              <span className="leading-relaxed">{product.whatItDoes}</span>
                            </div>
                            <div>
                              <strong className="text-[#1D1D1F] font-medium block">Intended users:</strong>
                              <span className="leading-relaxed">{product.whoItsFor}</span>
                            </div>
                            <div>
                              <strong className="text-[#1D1D1F] font-medium block">Technical edge:</strong>
                              <span className="leading-relaxed">{product.technicalDifferentiator}</span>
                            </div>
                          </div>

                          {/* Bulleted Specs */}
                          <div className="space-y-2 pt-3 border-t border-[#E8E8ED]/60">
                            <span className="text-[9px] font-semibold text-[#6E6E73] uppercase tracking-wider block">Operational Specs</span>
                            <ul className="space-y-1.5">
                              {product.specs.map((spec, sIdx) => (
                                <li key={sIdx} className="flex items-start gap-2 text-[11px] text-[#6E6E73] leading-relaxed">
                        ...
                        <div className="pt-4 border-t border-[#E8E8ED] flex items-center justify-between text-[11px] text-[#6E6E73] bg-[#F5F5F7] px-4 py-2.5 rounded-xl">
                          <div>
                            <span className="text-[#1D1D1F] font-medium block">System Latency</span>
                            <span>We deliver {product.latency}.</span>
                          </div>
                          <div className="text-right">
                            <span className="text-[#1D1D1F] font-medium block">Target Precision</span>
                        ...
                        <div className="pt-4">
                          <Link
                            to="/contact"
                            className="w-full py-2 rounded-full bg-[#1D1D1F] hover:bg-[#424245] text-white font-medium text-xs tracking-tight transition-colors flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                          >
                            {product.ctaText}
        ...
        {/* CTA Section */}
        <section className="rounded-[32px] bg-[#F5F5F7] p-8 sm:p-12 text-center space-y-6 border border-[#E8E8ED] relative overflow-hidden">
          <div className="max-w-2xl mx-auto space-y-4">
            ...
            <div className="flex flex-wrap justify-center items-center gap-4 pt-4">
              <Link
                to="/contact"
                className="px-6 py-2 rounded-full bg-[#0071E3] hover:bg-[#0055AB] text-white text-xs font-semibold tracking-tight transition-colors flex items-center gap-1.5 shadow-sm"
              >
                Book Discovery Call
              ...
              <Link
                to="/solutions"
                className="px-6 py-2 rounded-full bg-[#1D1D1F] hover:bg-[#424245] text-white text-xs font-semibold tracking-tight transition-colors"
              >
